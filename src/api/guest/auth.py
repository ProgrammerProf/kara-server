...
def register(request):
    ...
    user(
        ...
        login_host=client(request, 'host'), role=3, statistics=True,
...
def login(request):
    email = request.POST.get('email')
    password = request.POST.get('password')
    config = user.objects.filter(role=3, email=email, active=True, removed=False).first()
...
def change(request):
    ...
    config = user.objects.filter(id=token.user_id, role=3, active=True, removed=False).first()
    if not config: return response()

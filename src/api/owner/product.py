...
def default(request, usr):
    ...
    categories = [ch for ch in categories.values()]
...
def index(request, usr):
    ...
    if id: items = product.objects.filter(id=id, removed=False, owner_id=usr.id)
    else: items = product.objects.filter(removed=False, owner_id=usr.id).order_by('-id')
    data = []
    ...
    if id:
        ...
        categories = category.objects.filter(active=True, allow_products=True).order_by('-id')
        data['categories'] = [ch for ch in categories.values()]
...
def add(request, usr):
    ...
    product(
        ...
        update_date=get_date(), category_id=category_id, owner_id=usr.id,
...
def edit(request, usr):
    if not usr.see_products: return response()
    id = integer(request.POST.get('id'))
    config = product.objects.filter(id=id, removed=False, owner_id=usr.id).first()
